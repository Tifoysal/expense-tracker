    ...
    <div class="container mx-auto px-4 max-w-4xl">
        <div class="bg-white rounded-2xl shadow-2xl p-8">
            <div class="mb-10 text-center">
                <h2 class="text-3xl font-bold text-gray-800">Create Expense Category</h2>
                <p class="text-gray-500 mt-3 text-lg">Fill in the details to create a new expense category.</p>
            </div>
            ...
            <form action="{{ route('expenses.category.store') }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <select id="parent_id" name="parent_id" class="mt-2 block w-full rounded-lg border border-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-400 bg-gray-50 text-base py-2 px-4 focus:outline-indigo-600">
                            ...
                            @foreach ($categories as $cat)
                                ...
                                @foreach ($cat->childrenCategories as $childCategory)
                                    @include('backend.expense.expenses.child_category', ['child_category' => $childCategory, 'hi_pen' => $hi_pen])
                                @endforeach
                            @endforeach
                        </select>
                        @error('parent_id')
                    ...
                    <div>
                        ...
                        <input type="text" name="category_name" id="category_name" value="{{ old('category_name') }}" placeholder="Service Category Name" required class="mt-2 block w-full rounded-lg border border-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-400 bg-gray-50 text-base py-2 px-4 focus:outline-indigo-600">
                        @error('category_name')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
